...
const DetailSearch = (props: DetailSearchProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <DefaultWithMobile defaltValue={props.timeUnit} setValue={props.setTimeUnit} menuItems={menuItems} />
        <SelectDate startDate={props.startDate} setStartDate={props.setStartDate}/>
      </Box>
      <Box
        sx={{
          display: "flex",
      ...
      >
        <RadioButtons label={'디바이스'} radioArray={deviceRadioArray} value={props.device} onChange={props.setDevice}/>
        <RadioButtons label={'성별'} radioArray={genderRadioArray} value={props.gender} onChange={props.setGender}/>
      </Box>
    </Box>
  );
};
